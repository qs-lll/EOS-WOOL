...
class Cleos:
    ...
    def get_table(self, code, scope, table, table_key='', lower_bound='', upper_bound='', limit=10, timeout=30):
        ...
        '''
        json = {"json": True, "code": code, "scope": scope, "table": table, "table_key": table_key,
    ...
    def transfer(self, creator_privkey, ffrom, to,
                 quantity, memo='', permission='active',
                 contract='eosio.token', broadcast=True, timeout=30):
        # create delegatebw
        delegate_data = self.abi_json_to_bin(contract, 'transfer',
                                             {'from': ffrom, 'to': to, 'quantity': quantity,
                                              'memo': memo})
        tranfer_json = {
            'account': contract,
            'name': 'transfer',
    ...
    def delegatebw(self, creator, creator_privkey, acct_name,
                   stake_net='0.0001 EOS', stake_cpu='0.0001 EOS', permission='active',
    ...
    def undelegatebw(self, creator, creator_privkey, acct_name,
                     stake_net='0.0001 EOS', stake_cpu='0.0001 EOS', permission='active',
                     broadcast=True, timeout=30):
        # create undelegatebw
        delegate_data = self.abi_json_to_bin('eosio', 'undelegatebw',
                                             {'from': creator, 'receiver': acct_name, 'unstake_net_quantity': stake_net,
                                              'unstake_cpu_quantity': stake_cpu})
        delegate_json = {
            'account': 'eosio',
            'name': 'undelegatebw',
            'authorization': [
    ...
    def refund(self, owner, creator_privkey, permission='active', broadcast=True, timeout=30):
        # refund
        refund_data = self.abi_json_to_bin('eosio', 'refund',
                                             {'owner': owner})
        refund_json = {
            'account': 'eosio',
            'name': 'refund',
